const { token, botName, ver, execSpace, execUserID, execGroupID, pyChId, cChId, javaChId, nodeChId, nako3ChId } = require('./config.json');
...
const execCommand = (containerName) => {
    return new Promise((resolve, reject) => { // コマンド実行
        try {
            ...
            } else if (containerName == "c5") { // なでしこの場合
                exec(`docker run --name ${containerName} --rm -u ${execUserID}:${execGroupID} -v /etc/group:/etc/group:ro -v /etc/passwd:/etc/passwd:ro -v ${execSpace}/Containers/${containerName}/run:/run:ro rin/${containerName}`, { timeout: 10000 },
                    function (error, stdout, stderr) {
                        // シェル上でコマンドを実行できなかった場合のエラー処理
                        if (error !== null) {
...
client.on("messageCreate", async (msg) => {
    ...
    if ((msg.channel.id == pyChId) && (msg.content.substring(0, 3) == "!py")) { // !pで始まるメッセージのみ反応
        ...
        await Promise.all([makeFiles(message, "main.py", "c1"), execCommand("c1")]).then((res) => { // ファイル作成, 実行, コンテナ削除
    ...
    if ((msg.channel.id == cChId) && (msg.content.substring(0, 2) == "!c")) { // !cで始まるメッセージのみ反応
        ...
        await Promise.all([makeFiles(message, "main.c", "c2"), execCommand("c2")]).then((res) => {
            // res[0] = 状態コード, res[1] = エラー
            if (res[0][0] != 0) {
                msg.reply("[Serious] 例外が発生しました。開発者に報告してください。");
            } else {
    ...
    if ((msg.channel.id == nako3ChId) && (msg.content.substring(0, 6) == "!nako3")) { // !nakoで始まるメッセージのみ反応
        var message = msg.content.split('```'); // メッセージを```で分割 (Ex:[ '!py', 'print("helloWorld")', '', 'こんにちは', '' ])
        console.log(message.length);
        console.log(message);

        msg.channel.send("[Info] プログラム(なでしこv3)の実行を開始します");

        await Promise.all([makeFiles(message, "main.nako3", "c5"), execCommand("c5")]).then((res) => {
            // res[0] = 状態コード, res[1] = エラー
            if (res[0][0] != 0) {
                msg.reply("[Serious] 例外が発生しました。開発者に報告してください。");
            } else {
        ...
        await finishExec("rin/c5").then((res) => { // res[0] = 状態コード, res[1] = エラー
            if (res == 1) {
                console.log("OK: 残留コンテナ削除成功");
            } else if (res == 2) {
                msg.reply("[Serious] 終了処理で想定外のエラーが発生しました。開発者に報告してください。");
