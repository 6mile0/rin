const { token, botName, ver, execSpace, execUserID, execGroupID, pyChId, cChId, javaChId, nako3ChId } = require('./config.json');
...
client.on("messageCreate", async (msg) => {
    ...
    if ((msg.channel.id == nako3ChId) && (msg.content.substring(0, 6) == "!nako3")) { // jで始まるメッセージに反応
        var message = msg.content.split('```'); // メッセージを```で分割
        console.log(message.length);
        console.log(message);
        ...
        await Promise.all([makeFiles(message, "main.nako3", "c5"), execCommand("c5")])
            .then(messagingResultMakeAndExec(msg))
            .catch((e) => {
                console.log(e);
                // 定型エラー処理
